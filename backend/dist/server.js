...
app.put('/api/songs/:id', async (req, res) => {
  ...
  try {
    ...
    if (!updatedSong) return res.status(404).json({
...
app.delete('/api/songs/:id', async (req, res) => {
  ...
  try {
    ...
    if (!deletedSong) return res.status(404).json({
      message: 'Song not found'
    });
    res.status(200).json({
...
app.post('/api/playlists', async (req, res) => {
  const {
    name,
    email
  } = req.body; // 从请求体中获取播放列表的名称和用户email
...
app.delete('/api/playlists/:id', async (req, res) => {
  ...
  try {
    ...
    if (!deletedPlaylist) return res.status(404).json({
      message: 'Playlist not found'
    });
    res.status(200).json({
